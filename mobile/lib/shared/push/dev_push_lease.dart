...
class BuzzPushLeaseDescriptor {
  ...
  factory BuzzPushLeaseDescriptor.fromRelayInformation(
    ...
    if (!pushKinds.contains(buzzPushMessageKind)) {
      throw const FormatException('NIP-11 does not advertise kind 9 for push');
    }
...
void validateBuzzPushLeasePlaintext(
  ...
  if (plaintext['active'] != true) {
    throw const FormatException('development lease must be active');
  }
...
void _validateGrant(
  ...
  if (grant.relayOrigin != descriptor.origin) {
    throw FormatException(
      'Stored relay origin ${grant.relayOrigin} does not match descriptor origin ${descriptor.origin}',
    );
  }
